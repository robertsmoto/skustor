...
type Item struct {
	...
	Images []struct {
		...
		// need an id for the image
		// check and delete current images if image information is given
		// put image information here
		// url, alt, caption, featured
		// order
	}
}
...
type RawMaterialNodes struct {
	RawMaterialNodes []RawMaterial `json:"rawMaterialNodes" validate:"dive"`
}

func (s *RawMaterialNodes) JsonLoad(fileBuffer []byte) (err error) {
	json.Unmarshal(fileBuffer, &s)
	return err
}
...
type PartNodes struct {
	PartNodes []Part `json:"partNodes" validate:"dive"`
}

func (s *PartNodes) JsonLoad(fileBuffer []byte) (err error) {
	json.Unmarshal(fileBuffer, &s)
	return err
}
...
type ProductNodes struct {
	ProductNodes []Product `json:"productNodes" validate:"dive"`
}

func (s *ProductNodes) JsonLoad(fileBuffer []byte) (err error) {
	json.Unmarshal(fileBuffer, &s)
	return err
}
